...
parser_prole.add_argument('--redis_url',
                          default=os.environ.get('HOST', '127.0.0.1:6379/0'),
                          help='URL for Redis service, default: 127.0.0.1:6379/0')
parser_prole.add_argument('--workers', type=int,
                          default=number_of_proles(),
                          help='num rq workers, default: num_cores-2')
...
def red_october(args):
    from survey_stats.worker import launch_worker
    logging.info("launching %d rq workers" % (args.workers))
    workers = [launch_worker(None) for i in range(args.workers)]


parser_serve.set_defaults(func=serve_api)
parser_prole.set_defaults(func=red_october)
